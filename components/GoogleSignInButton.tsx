...
export function GoogleSignInButton({
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            logger.userAction('Google Sign In Button Success')
            onSuccess?.()
        } catch (error) {
            logger.error('Google Sign In Button Error', error)
    ...
    // Show different UI if Firebase is not configured
    if (!hasValidConfig) {
        return (
            <div className={`
                w-full flex items-center justify-center gap-3 
                px-4 py-3 border border-yellow-300 rounded-lg 
