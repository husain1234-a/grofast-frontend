...
app.add_event_handler(
    "startup",
    create_startup_event_handler("order-service", db_manager.get_db, settings)
)
...
# Register service dependency checks
async def check_cart_service():
    return await health_checker.check_http_service("cart-service", f"{settings.cart_service_url}/health")

health_checker.register_dependency_check("cart-service", check_cart_service)

async def check_notification_service():
    return await health_checker.check_http_service("notification-service", f"{settings.notification_service_url}/health")
